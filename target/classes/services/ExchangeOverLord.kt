...
data class CancelRejectCall(val orders: List<Order>, val exchangeId: String)
...
object ExchangeOverLord : IPrivateExchangeListener, IPublicExchangeListener {
    ...
    var bookBindings = HashMap<ExchangeName, HashMap<String, IPublicExchangeListener>>()
    var privateBindings = HashMap<String, HashMap<String, IPrivateExchangeListener>>()
    ...
    fun bind(
        ...
        bookBindings[exchangeName]!!.put(currency, listener)
    ...
    fun bindPrivate(
        ...
        if (exchangeId !in privateBindings) {
            privateBindings[exchangeId] = HashMap()
        }
        privateBindings[exchangeId]!!.put(currency, listener)
    ...
    override fun onCancelReject(orders: List<Order>, exchangeId: String) {
        try {
            ringBuffer.publishEvent { event, _ ->
                event.cancelRejectCall = CancelRejectCall(orders, exchangeId)
                event.walletUpdateCall = null
                event.orderBookUpdateCall = null
                event.orderResponseCall = null
                event.positionUpdateCall = null
    ...
    private fun processOrderResponseQueue(call: OrderResponseCall) {
        try {
            ...
            privateBindings[call.order.exchangeId]!![call.order.symbol]!!.onOrderResponse(call.order)
        } catch (exception: Exception) {
    ...
    private fun processOrderBookQueue(call: OrderBookUpdateCall): Boolean {
        try {
            ...
            bookBindings[call.book.contract.exchange]!![call.book.contract.symbol]!!.onOrderBookUpdation(call.book)
        } catch (exception: Exception) {
    ...
    private fun processPositionUpdateQueue(call: PositionUpdateCall) {
        try {
            ...
            privateBindings[call.position.exchangeId]!![call.position.symbol]!!.onPositionUpdate(call.position)
        } catch (exception: Exception) {
    ...
    private fun processWalletUpdateQueue(call: WalletUpdateCall) {
        try {
            ...
            privateBindings[call.position.exchangeId]!![call.position.symbol]!!.onWalletUpdate(call.position)
        } catch (exception: Exception) {
    ...
    private fun processCancelRejectQueue(call: CancelRejectCall) {
        try {
            ...
            privateBindings[call.exchangeId]!![call.orders.get(0).symbol]!!.onCancelReject(call.orders, call.exchangeId)
        } catch (exception: Exception) {
